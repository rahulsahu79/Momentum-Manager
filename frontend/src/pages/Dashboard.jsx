...
export default function DashboardPage() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!formData.title) return toast.error("⚠️ Title is required!", { theme: "colored" });
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <User style={{ color: "var(--icon-color)" }} size={20} />
        </div>
        <h1 className="text-xl font-bold">{`📝 To-Do Dashboard`}</h1>
      ...
      <main className="max-w-4xl mx-auto p-6">
        {/* FORM CARD */}
        <Card className="p-6 mb-6 transition-colors duration-300" style={{ backgroundColor: "var(--card-bg)" }}>
        ...
        {/* TASKS TABLE */}
        <Card className="p-6 overflow-x-auto transition-colors duration-300" style={{ backgroundColor: "var(--card-bg)" }}>
          ...
          {tasks.length > 0 ? (
            <table className="w-full border-collapse">
              ...
              <tbody>
                {tasks.map(task => (
                  <tr key={task.id} style={{ borderBottom: `1px solid var(--table-row-border)` }}>
